  ...
  return (
    <DashboardShell>
      <div className="space-y-10 text-slate-100">
        <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900/80 to-jade-900/40 p-10 shadow-[0_25px_55px_-15px_rgba(0,0,0,0.9)]">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.3),_transparent_60%)]" />
          <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl space-y-5">
              <p className="text-xs font-semibold uppercase tracking-[0.4em] text-jade-200">Suite contractual · Beta privada</p>
              <h1 className="text-4xl font-semibold text-white">Crea borradores blindados en minutos</h1>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-slate-200">
                <span className="flex items-center gap-2 rounded-full border border-white/20 px-4 py-2 backdrop-blur">
            ...
            <div className="grid gap-4 text-sm text-slate-200">
              <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur">
                <p className="text-xs uppercase tracking-[0.35em] text-slate-300">Tiempo estimado</p>
                <p className="text-2xl font-semibold text-white">2 min</p>
                <p className="text-xs text-slate-300">para obtener un borrador usable</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur">
                <p className="text-xs uppercase tracking-[0.35em] text-slate-300">Cobertura</p>
                <p className="text-2xl font-semibold text-white">+200 plantillas</p>
                <p className="text-xs text-slate-300">curadas para operaciones MX</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
          <section className="space-y-8 rounded-3xl border border-white/10 bg-slate-950/60 p-6 shadow-lg shadow-black/40 backdrop-blur">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Configura el contexto</p>
                <h2 className="text-2xl font-semibold text-white">Datos para el borrador</h2>
              </div>
              {isLoadingOptions && <p className="text-sm text-slate-400">Cargando catálogos…</p>}
            </div>

            {!isLoadingOptions && !empresas.length && (
              <div className="flex flex-col gap-2 rounded-2xl border border-amber-400/30 bg-amber-500/10 p-4 text-sm text-amber-100">
                <p className="font-semibold text-amber-200">Sin empresas disponibles</p>
                <p>
                ...
                {optionsErrorHint && <p className="text-amber-200/80">Detalle: {optionsErrorHint}</p>}
              </div>
            )}

            <form className="space-y-5" onSubmit={handleSubmit}>
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Empresa</label>
                <select
                  name="empresa"
                  value={formState.empresa}
                  onChange={handleChange}
                  required
                  className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/40 px-3 py-2 text-sm text-slate-100 focus:border-jade-400 focus:outline-none"
                >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Plantilla base (opcional)</label>
                  <select
                    name="template"
                ...
                <div className="grid gap-2 rounded-2xl border border-white/10 bg-slate-900/30 p-4 text-xs text-slate-300">
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    <select
                      ...
                      onChange={handleChange}
                      className="rounded-xl border border-white/10 bg-slate-950/40 px-3 py-2 text-sm text-slate-100 focus:border-jade-400 focus:outline-none"
                    >
              ...
              {selectedTemplate && (
                <div className="rounded-2xl border border-white/10 bg-slate-900/40 p-4 text-sm text-slate-100">
                  <p className="font-semibold text-white">{selectedTemplate.nombre}</p>
                  <p className="mt-1 text-slate-300">Clave: {selectedTemplate.clave}</p>
                  <p className="mt-1 text-slate-300">Proceso: {selectedTemplate.proceso}</p>
                  <p className="mt-1 text-slate-400">{selectedTemplate.descripcion || "Sin descripción"}</p>
                </div>
              )}

              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Resumen de necesidades</label>
                <textarea
                  ...
                  className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950/40 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:border-jade-400 focus:outline-none"
                />
              </div>

              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Cláusulas o requisitos especiales</label>
                <textarea
                  name="clausulas"
              ...
              <div className="flex flex-col gap-3 md:flex-row">
                <button
                  ...
                  className="flex-1 rounded-2xl bg-jade-400/90 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-jade-300 disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="rounded-2xl border border-white/15 px-4 py-3 text-sm font-medium text-slate-200 hover:border-white/40"
                >
                  Limpiar formulario
                </button>
              </div>
            ...
            <div className="rounded-3xl border border-white/5 bg-gradient-to-br from-indigo-900/40 via-slate-950/30 to-slate-950/50 p-5 shadow-inner shadow-black/40">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.35em] text-indigo-200">Biblioteca viva</p>
                  <h3 className="text-xl font-semibold text-white">Cláusulas sugeridas</h3>
                ...
                <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                  <input
                    ...
                    className="w-full rounded-2xl border border-white/10 bg-slate-950/40 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:border-indigo-300 focus:outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => void refreshClauseSuggestions(clauseQuery)}
                    disabled={isLoadingClauses || !formState.empresa}
                    className="rounded-2xl bg-indigo-500/80 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-400 disabled:opacity-60"
                  >
              ...
              {!formState.empresa ? (
                <p className="mt-5 rounded-2xl border border-dashed border-white/10 bg-white/5 p-4 text-sm text-slate-200">
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-5 shadow-inner shadow-black/30">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.35em] text-slate-400">Panel de redlines</p>
                  <h3 className="text-xl font-semibold text-white">Compara versiones y resalta riesgos</h3>
                </div>
                <button
                  type="button"
                  onClick={handleUseCurrentDraft}
                  disabled={!result}
                  className="rounded-2xl border border-white/15 px-4 py-2 text-xs font-semibold text-white hover:border-jade-300 disabled:opacity-40"
                >
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Texto original (base)</label>
                  <textarea
                    value={redlineBase}
                    onChange={(event) => setRedlineBase(event.target.value)}
                    rows={8}
                    placeholder="Pega aquí el contrato original o la versión generada"
                    className="mt-2 w-full rounded-2xl border border-white/15 bg-slate-950/50 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:border-jade-400 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Versión con comentarios / contraparte</label>
                  <textarea
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                <button
                  ...
                  className="rounded-2xl bg-white/15 px-5 py-2 text-sm font-semibold text-white transition hover:bg-white/25 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="rounded-2xl border border-white/15 px-5 py-2 text-sm font-semibold text-white hover:border-white/40"
                >
                  Limpiar panel
                </button>
              </div>
              ...
              <div className="mt-4">
                {redlineResult ? (
                  ...
                  <p className="rounded-2xl border border-dashed border-white/15 bg-white/5 p-4 text-sm text-slate-200">
                    Aún no procesamos redlines. Carga ambas versiones y presiona &quot;Analizar&quot; para diffs, riesgos y oportunidades
                    resumidas.
                  </p>
                )}
              </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-lg shadow-black/40">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Estado del generador</p>
                  <h3 className="text-2xl font-semibold text-white">
                    {isGenerating ? "Generando borrador" : result ? "Borrador listo" : "Aún sin generación"}
                  </h3>
                  <p className="text-sm text-slate-400">Seguimiento en tiempo real de cada iteración.</p>
                </div>
              ...
              {shouldShowTips ? (
                <div className="mt-5 rounded-2xl border border-jade-400/20 bg-jade-400/10 p-4 text-sm text-jade-100">
                  <p className="text-xs uppercase tracking-[0.35em] text-jade-200">Optimizando materialidad</p>
            ...
            <div className="space-y-4 rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-lg shadow-black/40">
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={handleCopy}
                  disabled={!result}
                  className="flex-1 rounded-2xl border border-white/15 px-4 py-2 text-sm font-semibold text-white hover:border-jade-300 disabled:opacity-40"
                >
                  Copiar borrador
                </button>
                <button
                ...
                <button
                  ...
                  className="flex-1 rounded-2xl bg-jade-400/80 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-jade-300 disabled:opacity-50"
                >
              ...
              <div className="rounded-2xl border border-white/10 bg-slate-900/40 p-4">
                {result ? (
                  <>
                    ...
                    <pre className="mt-3 max-h-[420px] overflow-auto whitespace-pre-wrap rounded-2xl bg-slate-950/70 p-4 text-xs text-slate-100">
            ...
            {hasCitations && result?.citas_legales ? (
              <div className="space-y-4 rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-lg shadow-black/40">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Referencias legales</p>
                    <h3 className="text-xl font-semibold text-white">Citas generadas para sustentar el borrador</h3>
                  </div>
...
function ClauseSuggestionCard({ clause, onInsert, onCopy }: ClauseSuggestionCardProps) {
  return (
    <article className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-950/70 p-4 shadow-inner shadow-black/40">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.3em] text-slate-400">{clause.nivel_riesgo}</p>
          <h4 className="text-lg font-semibold text-white">{clause.titulo}</h4>
        </div>
        <span className="rounded-full border border-white/15 px-3 py-1 text-xs text-slate-300">
          {Math.round(clause.relevancia * 100)}%
        </span>
      </div>
      <p className="text-sm text-slate-200 line-clamp-4">{clause.texto}</p>
      ...
      <div className="flex flex-wrap gap-2">
        {clause.palabras_clave.slice(0, 4).map((keyword) => (
          <span key={keyword} className="rounded-full border border-white/10 px-2 py-0.5 text-[11px] text-slate-300">
            {keyword}
          </span>
        ))}
      </div>
      <div className="flex flex-col gap-2 pt-2 sm:flex-row">
        <button
          ...
          className="flex-1 rounded-2xl bg-jade-500/80 px-3 py-2 text-sm font-semibold text-slate-950 transition hover:bg-jade-400"
        >
          Insertar en brief
        </button>
        <button
          ...
          className="flex-1 rounded-2xl border border-white/15 px-3 py-2 text-sm font-semibold text-white hover:border-jade-300"
        >
          Copiar texto
        </button>
      </div>
    ...
    <div className="space-y-3">
      {citations.map((citation) => (
        <article
          ...
          className="rounded-2xl border border-white/10 bg-slate-900/40 p-4 text-sm text-slate-200"
        >
          ...
          <p className="mt-1 text-slate-300">{citation.referencia}</p>
          {citation.resumen && <p className="mt-2 text-slate-400">{citation.resumen}</p>}
...
function CitationMetadataBadge({ metadata }: { metadata: CitationCacheMetadata }) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-jade-400/20 bg-jade-400/10 px-4 py-2 text-xs text-jade-100">
  ...
  return (
    <div className="space-y-4" aria-live="polite">
      <div className="rounded-2xl border border-white/15 bg-slate-900/40 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Alerta global</p>
            <p className="text-lg font-semibold text-white">{analysis.alerta_global}</p>
          </div>
          <span className="rounded-full border border-white/15 px-3 py-1 text-xs text-slate-200">
            {changePercent}% de cambio
          </span>
        </div>
        <p className="mt-2 text-sm text-slate-300">{analysis.resumen}</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
          <p className="text-xs uppercase tracking-[0.35em] text-amber-300">Riesgos clave</p>
          <ul className="mt-2 space-y-2 text-sm text-slate-200">
            {analysis.riesgos.map((risk) => (
              <li key={risk.titulo} className="rounded-xl border border-amber-400/30 bg-amber-400/10 p-3">
                <p className="text-sm font-semibold text-amber-100">{risk.titulo}</p>
                <p className="text-xs text-slate-900/80">Impacto: {risk.impacto}</p>
                {risk.detalle && <p className="text-xs text-slate-100/90">{risk.detalle}</p>}
                {risk.accion && <p className="text-xs text-white/80">Acción sugerida: {risk.accion}</p>}
              </li>
            ))}
          </ul>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
          <p className="text-xs uppercase tracking-[0.35em] text-jade-200">Oportunidades</p>
          <ul className="mt-2 space-y-2 text-sm text-slate-200">
            {analysis.oportunidades.map((opportunity) => (
              <li key={opportunity.titulo} className="rounded-xl border border-jade-400/30 bg-jade-400/10 p-3">
                <p className="text-sm font-semibold text-white">{opportunity.titulo}</p>
                <p className="text-xs text-jade-50/80">{opportunity.descripcion}</p>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="space-y-3">
        <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Diff inteligente</p>
...
function DiffSegmentBlock({ segment, index }: { segment: DiffSegment; index: number }) {
  const labelMap: Record<DiffSegment["type"], { label: string; color: string }> = {
    equal: { label: "Sin cambios", color: "text-slate-300" },
    insert: { label: "Insertado", color: "text-emerald-300" },
    delete: { label: "Eliminado", color: "text-rose-300" },
    replace: { label: "Modificado", color: "text-amber-200" },
  };
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <DiffColumn title="Original" color="text-slate-300" lines={segment.original} />
        <DiffColumn title="Revisado" color="text-white" lines={segment.revisado} />
      </div>
