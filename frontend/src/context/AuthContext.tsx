...
type AuthContextValue = {
  ...
  discoverOrganizations: (email: string, password: string) => Promise<Organization[]>;
};
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = useCallback(
    async ({ email, password, tenant: tenantSlug }: LoginPayload) => {
      ...
      try {
        response = await fetch(`${apiBaseUrl}/api/accounts/token/`, {
          method: "POST",
        ...
        if ((err as Error).name === "AbortError") {
          throw new Error("El servidor no respondió. Verifica que el sistema esté en línea.");
        }
  ...
  const discoverOrganizations = useCallback(
    async (email: string, password: string): Promise<Organization[]> => {
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 12_000);
      let response: Response;
      try {
        response = await fetch(`${apiBaseUrl}/api/accounts/my-organizations/`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error("Error al consultar organizaciones");
      }
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [accessToken, tenant, user, isProfileLoaded, login, logout, refreshProfile, refreshToken, discoverOrganizations]
  );
