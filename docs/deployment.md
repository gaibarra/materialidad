...
El script instala todo: paquetes del sistema, Node.js 20.x, Python 3.12, PostgreSQL, Nginx, systemd (con fallback a Supervisor para frontend), Certbot, clona el repo, construye el frontend y backend, y levanta todos los servicios.
...
Variable operativa recomendada para el cierre del pipeline FDI:
- `FDI_ALLOW_LEGACY_FALLBACK` — `True` durante validación; cambiar a `False` solo cuando staging pase los readiness gates y quieras apagar la degradación silenciosa al cálculo legacy.
...
## Frontend (Next.js con systemd; fallback Supervisor)
...
Instalar preferentemente el servicio de `systemd`:
```bash
sudo cp deploy/systemd/materialidad-frontend.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable materialidad-frontend
sudo systemctl restart materialidad-frontend
```

Verificar:
```bash
sudo systemctl status materialidad-frontend
```

Si por alguna razón no se usa `systemd`, existe fallback con Supervisor:
```bash
...
sudo supervisorctl start materialidad-frontend
```
...
## Snapshots FDI programados

La captura periódica de snapshots FDI debe correr con refresh previo de `OperationDefenseProjection`. En producción queda soportada por `systemd` con las units [deploy/systemd/materialidad-fdi-snapshots.service](deploy/systemd/materialidad-fdi-snapshots.service) y [deploy/systemd/materialidad-fdi-snapshots.timer](deploy/systemd/materialidad-fdi-snapshots.timer).

SLA operativo recomendado para este job:
- no permitir solapes entre ejecuciones
- abortar si supera 45 minutos
- monitorear `snapshot.lag_minutes`, `projection.lag_minutes` y `divergence.score_delta` desde el dashboard técnico

Instalación manual:

```bash
sudo cp deploy/systemd/materialidad-fdi-snapshots.service /etc/systemd/system/
sudo cp deploy/systemd/materialidad-fdi-snapshots.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable materialidad-fdi-snapshots.timer
sudo systemctl start materialidad-fdi-snapshots.timer
```
...
sudo /srv/materialidad/.venv/bin/python manage.py capture_fdi_snapshots --refresh-projections --days 90
```

La unit ya incluye `flock` para evitar solapes y `TimeoutStartSec=45m` para cortar ejecuciones fuera de SLA.
...
journalctl -u materialidad-frontend -f                  # Next.js (systemd)
journalctl -u materialidad-fdi-snapshots.service -f     # Batch snapshots FDI
...
sudo systemctl restart materialidad-frontend             # Frontend
